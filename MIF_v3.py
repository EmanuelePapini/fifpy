"""
 Multidimensional Iterative Filtering python package

 Dependencies : numpy, scipy, numba

 Wrapped from matlab version FIF2_v3.m 
...
def Settings(**kwargs):
    ...
    SPECIFIC PARAMETERS
     ...
     extensionType  (based on np.pad)
                    all standard np.pad mode extensions plus the following two
...
def MIF_run(x, options=None, M = np.array([]),**kwargs):
    ...
    return MIF_v3(x,options,M=M,**kwargs)
